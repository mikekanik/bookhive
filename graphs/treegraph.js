...
/** https://bl.ocks.org/d3noob/8375092 -  d3noob - Licensed under MIT */
function treegraph() {
    browser.storage.local.get(null).then(function (data) {
        const data_arr = Object.keys(data)
        let nodes = []

        treeData.push({ name: "All Bookmarks", parent: null, children: childData })
        ...
        for (let root of root_nodes) {
            childData.push({ name: root.name, parent: root.parent, children: root.children })
        }

        // ************** Generate the tree diagram	 *****************
        var margin = { top: 10, right: 120, bottom: 10, left: 100 },
            width = 4960 - margin.right - margin.left,
            height = 200 + (nodeCount*10) - margin.top - margin.bottom;
        ...
        function update(source) {
            ...
            nodeEnter.append("text")
                ...
                //Open URL if node is clicked
                .on('click', function (d) {
                    try {
                        if (d.url.includes("https") || d.url.includes("http"))
                            window.open(
                                d.url
                            )
                    } catch (err) {
                        console.log("Not a support Format " + err)
                    }
                });
